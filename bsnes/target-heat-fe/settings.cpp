...
const char* const DefocusNames[DefocusCount] = {"Pause emulation", "Block input", "Allow input"};

const char* const OutputNames[OutputCount] = {"Center (whole pixels)", "Scale (fill the window)",
...
// One line per setting, shared by load and save, so adding one is a single
// edit.
namespace {
struct IntField {
  const char* key;
...
void Settings::applyKey(const std::string& key, const std::string& value) {
  ...
  for(const IntField& f : IntFields) {
    if(key == f.key) {
      this->*f.field = SDL_clamp(number, f.min, f.max);
      return;
    }
  }
  for(const BoolField& f : BoolFields) {
    if(key == f.key) {
      this->*f.field = number != 0;
      return;
    }
  }
  for(const StrField& f : StrFields) {
    if(key == f.key) {
      this->*f.field = f.path ? normalPath(value) : value;
      return;
    }
  }
  ...
  if(key == "shaderparam") {
    ...
    if(equals != std::string::npos) {
      ...
      for(ShaderSetting& param : shaderParams) {
        if(param.name == name) {
          param.value = value.substr(equals + 1);
          return;
        }
      }
  ...
  if(key == "recent") {
    ...
    if(recent.size() < MaxRecent && std::find(recent.begin(), recent.end(), rom) == recent.end()) {
...
void Settings::save(const std::string& path) const {
  ...
  for(const ShaderSetting& param : shaderParams) {
    add("shaderparam", param.name + "=" + param.value);
  }
