...
uint8_t EmuCore::readMemory(MemoryDomain domain, uint32_t address) const {
  ...
  switch(domain) {
    ...
    case MemoryDomain::VRAM:
      return SuperFamicom::system.fastPPU() ? ppufast.debuggerReadVRAM(address)
                                            : ppu.debuggerReadVRAM(address);
    case MemoryDomain::CGRAM:
      return SuperFamicom::system.fastPPU() ? ppufast.debuggerReadCGRAM(address)
                                            : ppu.debuggerReadCGRAM(address);
    case MemoryDomain::OAM:
      return SuperFamicom::system.fastPPU() ? ppufast.debuggerReadOAM(address)
                                            : ppu.debuggerReadOAM(address);
    case MemoryDomain::APURAM:
  ...
  switch(domain) {
    ...
    case MemoryDomain::VRAM:
      if(SuperFamicom::system.fastPPU()) {
        ppufast.debuggerWriteVRAM(address, value);
      } else {
        ppu.debuggerWriteVRAM(address, value);
      }
      return;
    case MemoryDomain::CGRAM:
      if(SuperFamicom::system.fastPPU()) {
        ppufast.debuggerWriteCGRAM(address, value);
      } else {
        ppu.debuggerWriteCGRAM(address, value);
      }
      return;
    case MemoryDomain::OAM:
      if(SuperFamicom::system.fastPPU()) {
        ppufast.debuggerWriteOAM(address, value);
      } else {
        ppu.debuggerWriteOAM(address, value);
      }
      return;
    case MemoryDomain::APURAM:
      dsp.apuram[address] = value;
      return;
  }
}
