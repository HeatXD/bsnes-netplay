...
auto EmuCore::Impl::applyHacks() -> void {
  ...
  for(auto& hf : Hotfixes) {
    ...
    if(hf.disablePPU && ppuFast) {
      ppuFast = false;
      activeHotfix = {"PPU fast mode disabled for ", hf.title};
    }
    if(hf.disableDSP && dspFast) {
      dspFast = false;
      activeHotfix = {"DSP fast mode disabled for ", hf.title};
    }
  }
...
auto EmuCore::Impl::inputPoll(uint port, uint device, uint input) -> int16 {
  ...
  if(owner.onInputPoll) { return owner.onInputPoll((int)port, (int)device, (int)input, value); }
  return value;
}
...
bool EmuCore::setOption(const std::string& name, const std::string& value) {
  ...
  if(name == "Frontend/Hotfixes") {
    h.hotfixes = on;
    impl->applyHacks();
    return true;
  }
  if(name == "Hacks/PPU/Fast") {
    h.ppuFast = on;
    impl->applyHacks();
    return true;
  }
  if(name == "Hacks/PPU/NoSpriteLimit") {
    h.ppuNoSpriteLimit = on;
    impl->applyHacks();
    return true;
  }
  if(name == "Hacks/PPU/Mode7/Scale") {
    h.mode7Scale = std::atoi(value.c_str());
    impl->applyHacks();
    return true;
  }
  if(name == "Hacks/DSP/Fast") {
    h.dspFast = on;
    impl->applyHacks();
    return true;
  }
  if(name == "Hacks/DSP/Cubic") {
    h.dspCubic = on;
    impl->applyHacks();
    return true;
  }
  if(name == "Hacks/Coprocessor/DelayedSync") {
    h.coprocessorDelayedSync = on;
    impl->applyHacks();
    return true;
  }
  if(name == "Hacks/Coprocessor/PreferHLE") {
    h.coprocessorPreferHLE = on;
    impl->applyHacks();
    return true;
  }
...
// the core downgrades Fast to Strict itself where it corrupts
// (sfc/system/system.cpp:24)
...
// the core checks its header but not the buffer length, so a short blob reads
// off the end
...
bool EmuCore::unserialize(const std::vector<uint8_t>& state) {
  ...
  if(field(0) != 0x31545342) {
    return false;  // "BST1", as the core stamps it
  }
...
bool EmuCore::gameBoyLoaded() const { return loaded() && (bool)impl->gameBoy; }
