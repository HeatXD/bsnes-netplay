...
bool hexText(const std::string& text) {
  ...
  for(char ch : text) if(!std::isxdigit((unsigned char)ch)) return false;
  return true;
}
...
bool decodeSnes(std::string& code) {
  ...
  if(code.size() == 9 && code[6] == '=') {
    return hexText(code.substr(0, 6) + code.substr(7, 2));
  }
...
bool decodeGb(std::string& code) {
  ...
  if(code.size() == 7 && code[4] == '=') {
    return hexText(code.substr(0, 4) + code.substr(5, 2));
  }
